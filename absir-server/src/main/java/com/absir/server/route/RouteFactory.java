...
public class RouteFactory implements IBeanDefineSupply, IBeanFactoryAware, IMethodEntry<Object> {
	...
	public List<BeanDefine> getBeanDefines(BeanFactoryImpl beanFactory, Class<?> beanType) {
		...
		if (!BeanDefineOriginal.isAbstractBeanType(beanType)) {
			...
			if (server != null) {
				RouteDefine routeDefine = new RouteDefine(new InjectBeanDefine(new BeanDefineType(beanType), server.scope() == OnScope.SINGLETON ? BeanScope.SINGLETON : BeanScope.PROTOTYPE),
						server.scope());
		...
		for (int i = 0; i < length; i++) {
			...
			if (parameterResolvers != null) {
				for (ParameterResolver parameterResolver : parameterResolvers) {
					parameter = parameterResolver.getParameter(i, parameterNames, routeMethod.parameterTypes, parameterAnnotations, beanMethod);
					if (parameter != null) {
	 ...
	 * @return
	 */
	protected RouteEntity getRouteEntity(final Object beanObject, final BeanDefine beanDefine, RouteDefine routeDefine) {
