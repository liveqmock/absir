...
@Basis
@Bean
public class InjectBeanFactory implements IBeanFactorySupport, IBeanDefineSupply, IBeanDefineAware, IBeanObjectProcessor, IBeanFactoryAware, IAdapterSupport, IBeanFactoryStarted, IBeanFactoryStopping {
	...
	private IFieldSupport[] fieldSupports = new IFieldSupport[] { new IFieldSupport() {
		...
		public InjectInvoker getInjectInvoker(BeanScope beanScope, BeanDefine beanDefine, Field field) {
			...
			if (inject != null) {
				return InjectFieldOrders.getInjectField(field, inject.value(), inject.type(), field.getAnnotation(Orders.class));
			}
	...
	new IMethodInject<Stopping>() {
		...
		@Override
		public void setInjectMethod(Stopping inject, Method method, Object beanObject, InjectMethod injectMethod) {
			// TODO Auto-generated method stub
		...
		if (beanDefining) {
			...
			if (method == null) {
				synchronized (beanMethods) {
					method = beanMethods.get(beanMethod);
					if (method == null) {
						method = KernelReflect.declaredMethod(beanMethod.getBeanType(), beanMethod.getMethod().getName(), beanMethod.getMethod().getParameterTypes());
						if (method == null) {
							return null;
						}
	...
	public List<BeanDefine> getBeanDefines(final BeanFactoryImpl beanFactory, final Class<?> beanType, final BeanDefine beanDefine) {
		...
		KernelReflect.doWithDeclaredFields(beanType, new CallbackBreak<Field>() {
			...
			public void doWith(Field template) throws BreakException {
				// TODO Auto-generated method stub
				if (Modifier.isStatic(template.getModifiers()) && !Modifier.isFinal(template.getModifiers()) && template.getAnnotations().length > 0) {
		...
		KernelReflect.doWithDeclaredMethods(beanType, new CallbackBreak<Method>() {
			...
			public void doWith(Method template) throws BreakException {
				...
				if (beanDefineMethod == null || !template.getReturnType().isAssignableFrom(beanDefineMethod.getBeanType())) {
					if (beanMethod == null) {
						beanMethod = getBeanMethod(beanType, template);
						if (beanMethod == null) {
							return;
					...
					beanDefineMethod = new InjectBeanDefine((new BeanDefineMethod(beanName, beanDefine, beanMethod)), bean.scope());
	...
	/** injectObservers */
	private final Set<InjectObserver> injectObservers = Collections.newSetFromMap(new ConcurrentHashMap<InjectObserver, Boolean>());

	/** injectObserverObjects */
	private final Set<InjectObserverObject> injectObserverObjects = Collections.newSetFromMap(new ConcurrentHashMap<InjectObserverObject, Boolean>());
	...
	private void changedBeanDefine(BeanFactoryImpl beanFactory, BeanDefine beanDefine, boolean register) {
		...
		if (injectInvokerObserverSoftCaches.size() > 0) {
			for (Entry<InjectObserverSoftObject, List<SoftReference<Object>>> entry : injectInvokerObserverSoftCaches.entrySet()) {
	 ...
	 */
	@Override
	public void processBeanObject(final BeanFactory beanFactory, final BeanScope beanScope, final BeanDefine beanDefine, final Object beanObject, final Object beanProxy) {
		// TODO Auto-generated method stub
				...
				if (injectInvokers == null) {
						...
						public void doWith(Class<?> template) throws BreakException {
							...
							if (typeSupports != null) {
								for (ITypeSupport typeSupport : typeSupports) {
									InjectInvoker injectInvoker = typeSupport.getInjectInvoker(beanScope, beanDefine, template);
							...
							for (Field field : template.getDeclaredFields()) {
								if (!(Modifier.isStatic(field.getModifiers()) || Modifier.isFinal(field.getModifiers()))) {
									...
									for (IFieldSupport fieldSupport : fieldSupports) {
										InjectInvoker injectInvoker = fieldSupport.getInjectInvoker(beanScope, beanDefine, field);
										if (injectInvoker != null) {
											iInvokerScope.add(injectInvoker);
										}
									}
								...
								if (!Modifier.isStatic(method.getModifiers())) {
									...
									for (IMethodSupport methodSupport : methodSupports) {
										...
										if (inject != null) {
											...
											if (supports == null) {
												methodMapSupports.put(beanMethod, new HashSet<IMethodSupport>());
					...
					if (beanScope == BeanScope.PROTOTYPE) {
						...
						if (injectObserverSoftObject.injectObservers != null) {
							injectInvokerObserverSoftCaches.put(injectObserverSoftObject, new LinkedList<SoftReference<Object>>());
						}
	...
	public void afterRegister(BeanFactoryImpl beanFactory) {
		...
		if (injectAdapter != null) {
			for (IAdapterSupport adapterSupport : (List<IAdapterSupport>) (List) beanFactory.getBeanObjects(IAdapterSupport.class)) {
