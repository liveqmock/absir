...
public class BeanDefineType extends BeanDefineAbstractor {
	...
	public static <T> T getBeanObject(BeanFactory beanFactory, Constructor<T> constructor) {
		return getBeanObject(beanFactory, constructor, constructor.getParameterTypes(), BeanDefineDiscover.paramterNames(constructor));
	}
	 ...
	 * @return
	 */
	public static <T> T getBeanObject(BeanFactory beanFactory, Constructor<T> constructor, Class<?>[] paramTypes, String[] paramNames) {
		try {
			if (paramNames == null) {
