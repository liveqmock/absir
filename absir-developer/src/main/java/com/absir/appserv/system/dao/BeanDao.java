...
import com.absir.appserv.system.dao.hibernate.BaseDaoImpl;
...
public abstract class BeanDao {
	...
	public static <T> T find(Session session, Class<T> entityClass, Object id) {
		return get(session, entityClass, DynaBinder.to(id, SessionFactoryUtils.getIdentifierType(null, entityClass, session.getSessionFactory())));
	}
	...
	public static Object find(Session session, String entityName, Object id) {
		return get(session, entityName, DynaBinder.to(id, SessionFactoryUtils.getIdentifierType(entityName, null, session.getSessionFactory())));
	}
	...
	public static <T> T find(Session session, String entityName, Class<T> entityClass, Object id) {
		return get(session, entityName, entityClass, DynaBinder.to(id, SessionFactoryUtils.getIdentifierType(entityName, entityClass, session.getSessionFactory())));
	}
