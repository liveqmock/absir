...
public class UtilParameter {
	...
	public static InputStream getClassAsStream(Class<?> clazz) {
		...
		return CLASS_STREAM_READER.getClassAsStream(classLoader, clazz.getName());
	}
	...
	public static ClassReader getClassReader(Class<?> clazz) {
		InputStream byteCodeStream = getClassAsStream(clazz);
		if (byteCodeStream == null) {
			return null;
		}
	 ...
	 * @return
	 */
	public static String[] lookupParameterNames(ClassReader classReader, AccessibleObject methodOrCtor) {
		Class<?>[] types = null;
		String name = null;
		if (methodOrCtor instanceof Method) {
			Method method = (Method) methodOrCtor;
	 ...
	 * 
	 */
	public static interface ClassStreamReader {
		 ...
		 * @return
		 */
		public InputStream getClassAsStream(ClassLoader classLoader, String className);
	}

	/** Class_Stream_Reader */
	/** Class_Stream_Reader */
	public static final ClassStreamReader CLASS_STREAM_READER = new ClassStreamReader() {

		@Override
		public InputStream getClassAsStream(ClassLoader classLoader, String className) {
			// TODO Auto-generated method stub
			String name = className.replace('.', '/') + UtilPackage.CLASS_SUFFIX_NAME;
			InputStream asStream = classLoader.getResourceAsStream(name);
			if (asStream == null) {
				asStream = UtilParameter.class.getResourceAsStream(name);
			}
	...
	public static class Type {
		 ...
		 * or, for primitive types, the size, descriptor and getOpcode offsets
		 * for this type (byte 0 contains the size, byte 1 the descriptor, byte
		 * 2 the offset for IALOAD or IASTORE, byte 3 the offset for all other
		 * instructions).
		 */
		private int off;
