...
public final class BeanFactoryImpl implements BeanFactory {
	...
	private final Set<BeanSoftReference> beanSoftReferenceSet = Collections.newSetFromMap(new ConcurrentHashMap<BeanSoftReference, Boolean>());
	...
	public <T> T getBeanObject(String beanName, Class<T> beanType, boolean forcible) {
		...
		if (beanType.isArray()) {
			...
			if (beanObjects.isEmpty()) {
				if (forcible) {
					throw new RuntimeException("BeanName = " + beanName + " is " + beanObjects + " not match " + beanType);
				}
			}
	...
	private BeanDefine getBeanDefine(BeanDefine beanObject, String beanName, Class<?> beanType, float max) {
		...
		return beanFactoryImpl == null ? beanObject : beanFactoryImpl.getBeanDefine(beanObject, beanName, beanType, max);
	}
	...
	protected void replaceRegisteredBeanDefine(BeanDefine registeredBeanDefine, BeanDefine beanDefine) {
		if (beanNameDefineMap.containsKey(beanDefine.getBeanName())) {
			beanNameDefineMap.put(beanDefine.getBeanName(), beanDefine);
			if (registeredBeanDefine.getBeanScope() == BeanScope.PROTOTYPE || beanDefine.getBeanScope() == BeanScope.PROTOTYPE) {
				for (IBeanDefineAware beanDefineAware : beanDefineAwares) {
	...
	protected void unRegisterBeanDefineImpl(BeanDefine registeredBeanDefine, Object beanObject, BeanDefine beanDefine) {
		if (registeredBeanDefine != null) {
			if (beanDefine != null && !registeredBeanDefine.equals(beanDefine)) {
				throw new RuntimeException("BeanName + " + registeredBeanDefine.getBeanName() + " unRegister " + beanDefine + " not match " + registeredBeanDefine);

			} else if (beanObject != null) {
				if (!(registeredBeanDefine.getBeanScope() == BeanScope.PROTOTYPE ? registeredBeanDefine.getBeanType() == beanObject.getClass() : beanObject.equals(registeredBeanDefine
						.getBeanObject(this)))) {
					throw new RuntimeException("BeanName + " + registeredBeanDefine.getBeanName() + " unRegister " + beanDefine + " not match " + beanObject);
				}
			}
