...
<thead>
	<tr>
		...
		<%
			WebJsplDeveloper.setScenario("lookup", request);
		...
		%>
		<c:set var="node">
			<%=identifier + "\">" + field.getCaption() + "</th>"%>
		</c:set>
		<%
...
<tbody>
	<%
		DeveloperModel developerModel = DeveloperModel.forEntityNameClass(entityModel.getJoEntity(), request);
	%>
