...
public abstract class BeanDefineAbstract implements BeanDefine {
	 ...
	 * @return
	 */
	public static BeanDefine getBeanDefine(Class<?> beanType, String beanName, Object beanObject, BeanScope beanScope, BeanDefine beanDefine) {
		Class<?> beanClass = beanObject.getClass();
		if (beanScope == BeanScope.PROTOTYPE) {
			if (beanObject instanceof CloneTemplate) {
				return beanType == beanClass ? new BeanDefineCloneable(beanName, (CloneTemplate<?>) beanObject) : new BeanDefineCloneableType(beanType, beanName, (CloneTemplate<?>) beanObject);
			}
		...
		return beanType == beanClass ? new BeanDefineSingleton(beanName, beanObject) : new BeanDefineSingletonType(beanType, beanName, beanObject);
	}
}
