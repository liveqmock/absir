...
(function($){
	...
	$.fn.datepicker = function(opts){
		...
		return this.each(function(){
			var $this = $(this);
			/*K'naan@添加 自定义函数callfun 赋值日期时 执行callfun函数*/
			var callfun = $(this).attr('callfun');
			...
			function generateCalendar(dp){
				...
				$days.not('.disabled').click(function(){
					...
					if (!dp.hasTime()) {
						var date = dp.formatDate(dp.changeDay($day.attr("day"), $day.attr("chMonth")));
						closeCalendar();
						$this.val(date).focus();
						/*执行自定义函数*/
						if (callfun) {
							var fn = str2Func(callfun);
	                    	if (fn) fn.call(fn, $this, date);
						}
					} else {
			...
			$this.on('click', function(event){
				...
				$year.add($month).change(function(){
					if (dp.hasTime()) {
						var $day = $(setting.days$).find('.slt');
						dp.changeDateTime($year.val(), $month.val(), $day.attr('day'), dw.hour, dw.minute, dw.second);
						generateCalendar(dp);
					} else {
				...
				$(setting.okBut$).click(function(){
					...
					closeCalendar();
					$this.val(dp.formatDate(date)).focus();
					/*执行自定义函数*/
					if (callfun) {
						var fn = str2Func(callfun);
                    	if (fn) fn.call(fn, $this, dp.formatDate(date));
					}
				});
	...
	$.extend(Datepicker.prototype, {
		...
		/**K'naan@2014-04-23 添加changeDateTime**/
