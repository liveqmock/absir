	...
	$("form.required-validate", $p).each(function(){
		...
		$form.validate({
			...
			invalidHandler: function(form, validator) {
				...
				if (errors) {
//					$form.find(':input.error').filter(':first').focus();
	...
	// 这里放其他第三方jQuery插件...
	if ($.fn.selectedTodo) {
		$("a[target=selectedToExport]", $p).each(function(){
			var $this = $(this);
			var selectedIds = $this.attr("rel") || "ids";
			$this.click(function(){
				var targetType = $this.attr("targetType");
				var ids = $.fn.selectedTodo_getIds(selectedIds, targetType);
				if (!ids) {
					alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectMsg"));
					return false;
				}
				...
				function _doPost(){
					var form = $("<form>");   //定义一个form表单
				       ...
				       for(var idsi = 0; idsi < idsLength; idsi++) {
				    	   var selects = $('<input>');
					       selects.attr('type', 'hidden');
					       selects.attr('name', selectedIds);
					       selects.attr('value', ids[idsi]);
					       form.append(selects);
				       }
				       $('body').append(form);
				       form.submit();
...
(function($){
	...
	$.fn.extend({
			...
			function initSuffix($tbody) {
				$tbody.find('>tr').each(function(i){
					$(':input, a.btnLook, a.btnAttach', this).each(function(){
						...
						if (val && val.indexOf("#index#") >= 0) $this.val(val.replace('#index#',i));
					});
				});
			}
		...
		selectedTodo: function(){
			...
			$.fn.selectedTodo_getIds = function _getIds(selectedIds, targetType){
				var ids = "";
		...
		return this.each(function(){
			...
			$this.click(function(event){
				...
				$(setting.okBut$).click(function(){
					...
					$this.val(dp.formatDate(date)).change();
					closeCalendar();
				});
